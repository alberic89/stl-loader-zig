...
pub fn loadStl(allocator: Allocator, path: []const u8) ![]Triangle {
    const file = try std.fs.cwd().openFile(path, .{ .mode = .read_only });
    defer file.close();
    const reader = file.reader();

    var header_buffer: [5]u8 = undefined;
    _ = try reader.read(&header_buffer);

    return switch (std.mem.eql(u8, &header_buffer, "solid")) {
        true => {
            try file.seekTo(0);
            return try loadAscii(allocator, reader);
        },
        false => try loadBinary(allocator, reader),
    };
}

/// STL loader implementation for binary STL files (first 5 bytes != solid)
/// Accepts reader 5 bytes deep into the file (see load_stl)
pub fn loadBinary(allocator: Allocator, reader: anytype) ![]Triangle {
    try reader.skipBytes(75, .{});
    const facet_count = try reader.readInt(u32, .little);

    var mesh = try allocator.alloc(Triangle, facet_count);
    ...
    for (0..facet_count) |i| {
        ...
        const T = Triangle{
            .v1 = Point{
                .x = @as(f32, @bitCast(try reader.readInt(u32, .little))),
                .y = @as(f32, @bitCast(try reader.readInt(u32, .little))),
                .z = @as(f32, @bitCast(try reader.readInt(u32, .little))),
            },
            .v2 = Point{
                .x = @as(f32, @bitCast(try reader.readInt(u32, .little))),
                .y = @as(f32, @bitCast(try reader.readInt(u32, .little))),
...
pub fn loadAscii(allocator: Allocator, reader: anytype) ![]Triangle {
    ...
    while (o) |_| {
        ...
        for (0..3) |i| {
            if (try in_stream.readUntilDelimiterOrEof(&buf, '\n')) |b| {
                ...
                _ = coords.first();
                p[i] = Point{
                    .x = try std.fmt.parseFloat(f32, coords.next().?),
                    .y = try std.fmt.parseFloat(f32, coords.next().?),
                    .z = try std.fmt.parseFloat(f32, coords.next().?),
                };
            }
        }
